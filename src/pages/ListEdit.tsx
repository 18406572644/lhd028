...
import { DeleteOutlined, SearchOutlined, PlusOutlined, CloseOutlined, MenuOutlined } from '@ant-design/icons';
...
export default function ListEdit() {
  ...
  const { currentList, loading, fetchListDetail, updateList } = useMovieListStore();
  ...
  useEffect(() => {
    if (id) fetchListDetail(Number(id));
  }, [id]);
  ...
  useEffect(() => {
    if (currentList) {
      ...
      setVisibility(currentList.visibility);
      setCover(currentList.cover || '');
      setItems(
        (currentList.items || []).map((item) => ({
          movieId: item.movieId,
          title: item.movie?.title || `电影 #${item.movieId}`,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await updateList(Number(id), data);
      if (result) {
        message.success('片单更新成功');
        navigate(`/lists/${id}`);
      }
    } finally {
      setSubmitting(false);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="font-serif text-retro-cream text-2xl">编辑片单</h1>
        <div className="flex gap-2">
          <Button onClick={() => navigate(`/lists/${id}`)}>取消</Button>
        ...
        <div className="flex gap-4 flex-wrap">
          ...
          <div className="flex-1 min-w-[200px]">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-retro-gold-dark text-xs">点击电影海报设为封面</span>
            </div>
          </div>
        </div>
      </div>
              ...
              {items.map((item, index) => (
                ...
                >
                  ...
                  <div className="w-10 h-14 rounded overflow-hidden bg-retro-brown-dark shrink-0 cursor-pointer"
                    onClick={() => item.posterUrl && setCover(item.posterUrl)}
                    title="点击设为封面">
                    {item.posterUrl ? (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-retro-gold-dark text-xs mt-0.5">★ {item.rating}</div>
                  </div>
